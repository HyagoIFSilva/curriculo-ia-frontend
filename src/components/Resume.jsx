...
const Resume = forwardRef(({ dados }, ref) => {
  ...
  const renderContent = () => {
    ...
    const languagesSection = (<div className="sidebar-section"><h2>Idiomas</h2><div className="languages-container">{idiomas.map((lang, index) => (<div key={index} className="language-item"><p><strong>{lang.idioma}:</strong> {lang.nivel}</p></div>))}</div></div>);
    const skillsSection = (<div className="sidebar-section"><h2>Habilidades</h2><div className="skills-container-sidebar">{habilidades.split(',').map((skill, index) => (skill.trim() && <span key={index} className="skill-tag-sidebar">{skill.trim()}</span>))}</div></div>);
    ...
    switch (template) {
      case 'layout-creative-minimalist':
        return (<>
          ...
          <main className="resume-main-content">{sections}
      ...
      case 'layout-data-centric':
        return (<>
          ...
          <div className="content-grid">
            ...
            <div className="grid-item resume-languages">{languagesSection}</div>
            <div className="grid-item resume-main-col">{sections}</div>
          </div>
        </>);
  ...
  return (<div className="resume-container"><div ref={ref} className={`resume-preview ${template}`}>{renderContent()}</div></div>);
});
